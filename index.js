...
// ================== XP CONFIG ==================
const THRESHOLDS = { 1:0, 2:1000, 3:1500 };
...
client.on(Events.MessageCreate, async (message)=>{
  ...
  client.on(Events.MessageCreate, async (message)=>{
    ...
    if(!content.startsWith('!perfil')) return;

    const args = content.split(/\s+/);
    const targetMention = message.mentions.members.first();
    const data = await loadData();

    // Si mencionas a alguien, usamos ese usuario; si no, tu perfil
    const target = targetMention || member;

    // Comprobar si es staff o dueño si estás viendo otro perfil
    if(target.id !== member.id && !hasStaffPerm(member)){
      return message.reply('❌ No tienes permiso para ver otros perfiles.');
    }

    const key = `${message.guild.id}_${target.id}`;
    const rec = data.users[key] || { xp:0, level:1 };
    const from = THRESHOLDS[rec.level] || 0;
    const to = THRESHOLDS[rec.level+1] || THRESHOLDS[3]+200;
    const { bar, percent } = buildProgressBar(rec.xp, from, to);

    const embed = new EmbedBuilder()
      .setTitle(`📋 Perfil de ${target.user.tag}`)
      .setThumbnail(target.user.displayAvatarURL({ dynamic:true }))
      .addFields(
        { name:'Nivel', value:`${rec.level}`, inline:true },
        { name:'XP', value:`${rec.xp}/${to}`, inline:true },
        { name:'Progreso', value:`${bar} ${percent}%`, inline:false },
        { name:'Roles', value:target.roles.cache.map(r=>r.toString()).join(', ') || 'Ninguno', inline:false }
      )
      .setColor('#00FF00')
      .setTimestamp();
